...
<!-- Template /skins/amx/browse_main_details.template.php begin -->
...
<table align="center" style="width: 100%; margin-top: 5px;" border="0" cellspacing="0" cellpadding="0">
	<tr valign="bottom">
		...
		<td style="text-align: <?php echo __("left"); ?>;">
			<input type="hidden" type="text" name="directory" value="<?php echo $directory_html; ?>" style="width: 400px;" title="(accesskey g)" accesskey="g" /> 
...
<?php				if ($net2ftp_settings["functionuse_findstring"]     == "yes") { ?><input type="button" class="smallbutton" value="<?php echo __("Search"); ?>"         onclick="submitBrowseForm('<?php echo $directory_js; ?>', '', 'findstring', '');"           title="<?php echo __("Find files which contain a particular word"); ?> (accesskey f)" accesskey="f" /> <?php } // end if ?>
	...
	<tr class="browse_rows_heading">
		...
		<td colspan="1"><a style="text-decoration: underline; cursor: pointer; cursor: hand;" onclick="<?php echo $sortArray["permissions"]["onclick"]; ?>" title="<?php echo $sortArray["permissions"]["title"]; ?>"><?php echo $sortArray["permissions"]["text"]; ?></a><?php echo $sortArray["permissions"]["icon"]; ?></td>	--><td></td><td></td><td></td>	
		...
		<!-- <td colspan="3"><?php echo __("Actions"); ?></td>--><td colspan="3"></td>
	</tr>
...
<?php					printMime("type", $list_directories[$i]); ?>
				...
				<td><?php echo $list_directories[$i]["permissions"]; ?></td>--><td></td><td></td><td></td>
				<td><?php echo $list_directories[$i]["mtime"]; ?>
				<td colspan="3"></td>
			</tr>
<?php		} // end for ?>
<?php	} // end if ?>
...
<?php					printMime("type", $list_files[$i]); ?>
				...
				<td><?php echo $list_files[$i]["permissions"]; ?></td> --><td></td><td></td><td></td>
...
<?php					printMime("type", $list_symlinks[$i]); ?>
				...
				<td><?php echo $list_symlinks[$i]["permissions"]; ?></td>--> <td></td><td></td><td></td>
				<td><?php echo $list_symlinks[$i]["mtime"]; ?></td>
                                <!--
<?php				if ($net2ftp_settings["functionuse_view"]   == "yes") { ?><td></td><?php } ?>
<?php				if ($net2ftp_settings["functionuse_edit"]   == "yes") { ?><td></td><?php } ?>
<?php				if ($net2ftp_settings["functionuse_update"] == "yes") { ?><td></td><?php } ?>
<?php				if ($net2ftp_settings["functionuse_open"]   == "yes") { ?><td></td><?php } ?> --><td colspan="3"></td>
			</tr>
<?php		} // end for ?>
<?php	} // end if ?>
...
<!-- Template /skins/amx/browse_main_details.template.php end -->
