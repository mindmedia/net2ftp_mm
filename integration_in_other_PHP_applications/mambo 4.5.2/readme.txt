...
INTEGRATION INSTRUCTIONS FOR NET2FTP AND MAMBO 4.5.1 and 4.5.2
...
4 - Install the net2ftp module using Mambo's admin panel:
        - Go to Modules > Install/Uninstall
    ...
    Install the net2ftp component in a similar way:
        - Go to Components > Install/Uninstall
        - Under the heading "Install from directory", select the directory 
          containing the net2ftp component files /temporary/com_net2ftp
        - Click on "Install"

5 - MODIFICATIONS MUST BE MADE TO 2 MAMBO FILES:
	/index.php (to suppress the HTTP headers sent by Mambo)
	/includes/frontend.php (to print the CSS & Javascript needed by net2ftp)

    These files are provided in the directory /mambo_files_to_modify.
